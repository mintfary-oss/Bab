...
def moderation_dashboard_view(request: HttpRequest) -> HttpResponse:
    ...
    if bd and os.path.isdir(bd):
        for f in sorted(os.listdir(bd), reverse=True):
            if f.endswith(".gz"):
                backups.append(
                    {"name": f, "size_mb": round(os.path.getsize(os.path.join(bd, f)) / 1048576, 2)}
                )
    return render(
        request,
        "admin_panel/dashboard.html",
        {"pending_reports": pending, "pending_count": pending.count(), "backups": backups},
    )
...
def report_detail_view(request: HttpRequest, report_id: int) -> HttpResponse:
    ...
    if request.method == "POST":
        ...
        AdminActionLog.objects.create(
            ...
            action_type=AdminActionLog.ActionType.RESOLVE_REPORT,
            description=f"Жалоба #{report.pk}: {action}",
            target_type="report",
            target_id=report.pk,
        )
...
def backup_create_view(request: HttpRequest) -> HttpResponse:
    bd = getattr(settings, "BACKUP_DIR", "")
    if not bd:
        dm.error(request, "BACKUP_DIR не настроен.")
        return redirect("admin_panel:dashboard")
...
def backup_download_view(request: HttpRequest, filename: str) -> HttpResponse | FileResponse:
    ...
    fp = os.path.join(bd, safe)
    if not os.path.isfile(fp) or not safe.endswith(".gz"):
        dm.error(request, "Не найден.")
        return redirect("admin_panel:dashboard")
