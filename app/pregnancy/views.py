...
def pregnancy_dashboard_view(request: HttpRequest) -> HttpResponse:
    ...
    if not p:
        return redirect("pregnancy:create")
    w = p.current_week
    return render(
        request,
        "pregnancy/dashboard.html",
        {
            "pregnancy": p,
            "week": w,
            "week_info": get_week_info(w),
            "checklist": WeeklyChecklist.objects.filter(pregnancy=p, week=w),
        },
    )
    ...
    if request.method == "POST":
        ...
        if dd:
            ...
            for wn, info in WEEKS_DATA.items():
                for task in info["checklist"]:
                    items.append(WeeklyChecklist(pregnancy=p, week=wn, title=task))
            WeeklyChecklist.objects.bulk_create(items)
            messages.success(request, "Трекер создан!")
...
def week_detail_view(request: HttpRequest, week_num: int) -> HttpResponse:
    ...
    return render(
        request,
        "pregnancy/week_detail.html",
        {
            "pregnancy": p,
            "week_num": week_num,
            "week_info": wi,
            "checklist": WeeklyChecklist.objects.filter(pregnancy=p, week=week_num),
            "is_current": p.current_week == week_num,
        },
    )
...
def weeks_overview_view(request: HttpRequest) -> HttpResponse:
    ...
    return render(
        request,
        "pregnancy/weeks_overview.html",
        {"pregnancy": p, "weeks_range": list(range(1, 43))},
    )
