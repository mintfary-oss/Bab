...
class MediaUploadAPIView(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        try:
            validate_uploaded_file(uf, mt)
        except Exception as e:
            return Response({"error": str(e)}, status=status.HTTP_400_BAD_REQUEST)
        wi, hi = (None, None)
        if mt in ("image", "gif"):
            wi, hi = get_image_dimensions(uf)
        m = MediaFile.objects.create(
            owner=request.user,
            file=uf,
            media_type=mt,
            original_name=uf.name or "",
            mime_type=uf.content_type or "",
            file_size=uf.size or 0,
            width=wi,
            height=hi,
        )
        tp = os.path.join(settings.MEDIA_ROOT, "thumbnails", f"thumb_{m.pk}.jpg")
        if mt in ("image", "gif") and create_image_thumbnail(m.file.path, tp):
            m.thumbnail = f"thumbnails/thumb_{m.pk}.jpg"
            m.save(update_fields=["thumbnail"])
        elif mt == "video" and create_video_thumbnail(m.file.path, tp):
            m.thumbnail = f"thumbnails/thumb_{m.pk}.jpg"
            m.save(update_fields=["thumbnail"])
        return Response(
            MediaFileSerializer(m, context={"request": request}).data,
...
class MyMediaListAPIView(generics.ListAPIView):  # type: ignore[type-arg]
    ...
    def get_queryset(self):  # type: ignore[override]
        qs = MediaFile.objects.filter(owner=self.request.user)
