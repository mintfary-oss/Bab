...
class HospitalListAPIView(generics.ListAPIView):  # type: ignore[type-arg]
    serializer_class=HospitalSerializer;permission_classes=[permissions.AllowAny]
...
class HospitalDetailAPIView(generics.RetrieveAPIView):  # type: ignore[type-arg]
    serializer_class=HospitalSerializer;permission_classes=[permissions.AllowAny];queryset=Hospital.objects.filter(is_active=True)
class HospitalReviewListCreateAPIView(generics.ListCreateAPIView):  # type: ignore[type-arg]
    ...
    def perform_create(self,s): s.save(author=self.request.user,hospital_id=self.kwargs["hospital_id"])
