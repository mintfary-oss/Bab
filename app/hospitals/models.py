...
class Hospital(models.Model):
    ...
    hospital_type = models.CharField(
        max_length=10, choices=HospitalType.choices, default=HospitalType.PUBLIC
    )
...
class HospitalReview(models.Model):
    ...
    author = models.ForeignKey(
        settings.AUTH_USER_MODEL, on_delete=models.CASCADE, related_name="hospital_reviews"
    )
    ...
    def save(self, *a, **kw):
        ...
        s = HospitalReview.objects.filter(hospital=self.hospital).aggregate(
            avg=Avg("rating"), cnt=Count("id")
        )
        self.hospital.rating_avg = s["avg"] or 0
        self.hospital.reviews_count = s["cnt"] or 0
