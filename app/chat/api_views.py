...
class FriendRequestAPIView(APIView):
    ...
    def post(self, request: Request) -> Response:
        ...
        return Response(
            ...
            status=status.HTTP_201_CREATED if c else status.HTTP_200_OK,
        )
...
class ChatListAPIView(generics.ListAPIView):  # type: ignore[type-arg]
    ...
    def get_queryset(self):  # type: ignore[override]
        return Chat.objects.filter(
            pk__in=ChatMember.objects.filter(user=self.request.user).values_list(
                "chat_id", flat=True
            )
        )
...
class MessageListCreateAPIView(generics.ListCreateAPIView):  # type: ignore[type-arg]
    ...
    def perform_create(self, s):
        s.save(sender=self.request.user, chat_id=self.kwargs["chat_id"])
