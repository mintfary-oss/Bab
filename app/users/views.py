...
def profile_view(request: HttpRequest, user_id: int) -> HttpResponse:
    ...
    return render(
        request,
        "users/profile.html",
        {"profile_user": u, "profile": p, "is_own": request.user.pk == u.pk},
    )
...
def profile_edit_view(request: HttpRequest) -> HttpResponse:
    ...
    if request.method == "POST":
        uf = UserProfileForm(request.POST, request.FILES, instance=request.user)
        pf = ProfileDetailForm(request.POST, instance=p)
        if uf.is_valid() and pf.is_valid():
            uf.save()
            pf.save()
            return redirect("users:profile", user_id=request.user.pk)
    else:
        uf = UserProfileForm(instance=request.user)
        pf = ProfileDetailForm(instance=p)
