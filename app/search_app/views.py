    ...
    if q and len(q) >= 2:
        ...
        if st in ("all", "users"):
            results["users"] = User.objects.filter(
                Q(username__icontains=q) | Q(first_name__icontains=q), is_active=True
            )[:20]
        if st in ("all", "hospitals"):
            results["hospitals"] = Hospital.objects.filter(
                Q(name__icontains=q) | Q(address__icontains=q) | Q(region__icontains=q),
                is_active=True,
            )[:20]
        if st in ("all", "groups"):
            results["groups"] = Group.objects.filter(
                Q(title__icontains=q), privacy=Group.Privacy.PUBLIC
            )[:20]
    return render(
        request,
        "search_app/results.html",
        {
            "query": q,
            "search_type": st,
            "results": results,
