...
def feed_view(request: HttpRequest) -> HttpResponse:
    posts = (
        Post.objects.filter(status=Post.Status.PUBLISHED, visibility=Post.Visibility.PUBLIC)
...
def post_detail_view(request: HttpRequest, post_id: int) -> HttpResponse:
    ...
    liked = (
        request.user.is_authenticated and Like.objects.filter(post=post, user=request.user).exists()
    )
    return render(
        request,
        "posts/detail.html",
        {"post": post, "comments": comments, "comment_form": CommentForm(), "user_liked": liked},
    )
...
def my_posts_view(request: HttpRequest) -> HttpResponse:
    return render(
        request, "posts/my_posts.html", {"posts": Post.objects.filter(author=request.user)}
    )
